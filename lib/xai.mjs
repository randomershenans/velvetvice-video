import { expressifyForTTS } from './expressive.mjs';
...
PACING (this matters — the body is read aloud, so the punctuation IS the direction):
- Short, deliberate sentences. The narrator pauses on every period.
- Use ellipses (...) freely on charged or trailing moments — the voice draws out a real pause there.
- Use em-dashes (—) for caught-breath interruptions mid-sentence.
- Lean on delivery verbs like whispered, murmured, breathed, sighed — they cue a hushed register.
...
The first-person narrator's gender is given on each request — write the body's pronouns and self-references accordingly. If the narrator is a woman, she sees / wants / breathes / aches; the romantic counterpart is described with "he". If the narrator is a man, the inverse.

HEAT — each request names a level. It sets how far the physical charge goes. The hard rules above (suggestive never explicit; no anatomy; no profanity; nothing a mainstream platform would remove) ALWAYS hold, at EVERY level including the highest — this posts publicly to TikTok and Instagram.
- sweet: yearning and restraint. The almost-touch, charged glances, a hand that doesn't quite land.
- warm: first contact. A hand at the waist, breath against the neck, a kiss that unravels. Clothed, aching.
- steamy: heat with intent. Bodies pressed close, mouths, the slow drag of want, a barrier beginning to give — implication, not description.
- scorching: the edge of the moment. Maximum charge — caught breath, the last restraint about to break, the second before everything changes. Cut exactly at the peak. Everything is felt; nothing is shown.
...
export async function generateStory({ vibe, heat, narrator }) {
  const narratorGender = narrator === 'male' ? 'male' : 'female';
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: STORY_SYSTEM },
        {
          role: 'user',
          content:
            `Vibe / trope / setting: ${vibe}\n` +
            `Heat level: ${heat}\n` +
            `Narrator (first person): a ${narratorGender}\n\n` +
            `Write the package. The body must be told from the ${narratorGender} narrator's POV — keep pronouns and self-references consistent.`,
        },
      ],
    }),
  });
...
export async function generateAudio(text, voice) {
  ...
  // Wrap the body in xAI speech tags so the voice stays in a hushed, sensual
  // register with deliberate pauses on punctuation. See lib/expressive.mjs.
  const expressive = expressifyForTTS(text);
  const res = await fetch(`${XAI_BASE}/tts`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${key()}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      text: expressive.slice(0, MAX_TTS_CHARS),
      voice_id,
      language: 'en',
    }),
  });
